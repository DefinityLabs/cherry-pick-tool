...
module.exports = {
  ...
  writeFile: function() {
    fs.writeFile('.cherrypick', JSON.stringify(file), function(err){
      if (err) {
        console.error(emoji.get('shit') + '  error writing .cherrypick file, check the privileges or if the file is already open'.red, err);
      } else {
  ...
  load: function() {
    fs.access('.cherrypick', function(err){
      if (err) {
        console.error(emoji.get('see_no_evil') + '  there is no .cherrypick file'.cyan);
        console.log('');
        return;
      }
      fs.readFile('.cherrypick', function(err, data){
        if (err) {
          console.error(emoji.get('shit') + '  error reading .cherrypick file, check the privileges or if the file is already open'.red, err);
        } else {
          clearCherryPickFile();
          loadCherryPickFile(data);
          console.log(emoji.get('thumbsup') + '  cherry pick file was loaded!!!'.cyan);
        }
        console.log('');
      });
    });

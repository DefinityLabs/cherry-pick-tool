...
module.exports = {
  ...
  writeFile: function() {
    fs.writeFile('.cherrypick', JSON.stringify(file), function(err){
      if (err) {
        out.println(emoji.get('shit'), 'error writing .cherrypick file, check the privileges or if the file is already open'.red);
      } else {
  ...
  load: function() {
    fs.access('.cherrypick', function(err){
      ...
      fs.readFile('.cherrypick', function(err, data){
        if (err) {
          out.println(emoji.get('shit'), 'error reading .cherrypick file, check the privileges or if the file is already open'.red);
        } else {
          clearCherryPickFile();

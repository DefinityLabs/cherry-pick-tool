...
describe('printer', () => {
  ...
  beforeEach(() => {
    commit = {
      ...
      status: ['M', 'D', 'A', 'X'],
      files: ['index.js', 'lib/removed.js', 'lib/added.js', 'lib/other.js']
    };
    ...
    describe('without display position and files', () => {
      describe('when current timezone is the commit timezone', () => {
        ...
        it('displays the commit hash', () => {
          expect(out.println.mock.calls[0][0]).toEqual(('commit ' + commit.hash).yellow);
        ...
        it('displays the author information', () => {
          ...
          expect(out.println.mock.calls[1][2]).toEqual(' <' + commit.authorEmail + '>');
        });

        it('displays date information', () => {
          expect(out.println.mock.calls[2][0]).toEqual('Date:   ');
          expect(out.println.mock.calls[2][1]).toEqual('Sun Jan 01, 10:00:00AM');
          expect(out.println.mock.calls[2][2]).toEqual(' (' + commit.authorDateRel.magenta + ')');
        });
      ...
      describe('when current timezone is not the commit timezone', () => {
        describe('when the current time and commit time are different', () => {
          ...
          it('displays date information with original', () => {
            expect(out.println.mock.calls[2][0]).toEqual('Date:   ');
            expect(out.println.mock.calls[2][1]).toEqual(
              'Sun Jan 01, 09:00:00AM' + ' (10:00:00AM +01:00)'.gray
            );
            expect(out.println.mock.calls[2][2]).toEqual(' (' + commit.authorDateRel.magenta + ')');
          });
        });
        ...
        describe('when the current date and commit date are different', () => {
          ...
          it('displays date information with original', () => {
            expect(out.println.mock.calls[2][0]).toEqual('Date:   ');
            expect(out.println.mock.calls[2][1]).toEqual(
              'Mon Jan 02, 06:00:00AM' + ' (Sun Jan 01, 06:00:00PM +01:00)'.gray
            );
            expect(out.println.mock.calls[2][2]).toEqual(' (' + commit.authorDateRel.magenta + ')');
          });
        });
